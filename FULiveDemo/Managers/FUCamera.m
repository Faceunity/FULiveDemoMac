...
- (void)changeCameraPositionWithStr:(NSString *)deviceStr{
    ...
    if ([self.cameraInput.device.localizedName isEqualToString:deviceStr]) return;
    [self.captureSession stopRunning];
    [self.captureSession removeInput:self.cameraInput];
    NSError *error = nil;
    AVCaptureDevice *device = nil;
    for (AVCaptureDevice *device0 in devices) {
        if ([device0.localizedName isEqualToString:deviceStr]) {
            device = device0;
            break;
        }
    }
    self.cameraInput = [[AVCaptureDeviceInput  alloc] initWithDevice:device error:&error];
