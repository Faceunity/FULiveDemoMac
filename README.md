...
[FURenderer itemSetParam:items[0] withName:@"filter_name" value:@"origin"];
```
...
[FURenderer itemSetParam:items[0] withName:@"color_level" value:@(0.5)];
```
...
[FURenderer itemSetParam:items[0] withName:@"blur_level" value:@(6.0)];

//  Set item parameters - use old blur
[FURenderer itemSetParam:items[0] withName:@"use_old_blur" value:@(1.0)];
```
...
[FURenderer itemSetParam:items[0] withName:@"face_shape" value:@(3.0)];
```
...
[FURenderer itemSetParam:items[0] withName:@"face_shape_level" value:@(1.0)];
```
...
[FURenderer itemSetParam:items[0] withName:@"eye_enlarging" value:@(1.0)];
```
...
[FURenderer itemSetParam:items[0] withName:@"cheek_thinning" value:@(1.0)];
```

### 五、平台相关

PC或MAC端的美颜，使用前必须将参数 is_opengl_es 设置为 0，移动端无需此操作：

```C
//  Set item parameters
[FURenderer itemSetParam:items[0] withName:@"is_opengl_es" value:@(0)];
```
...
注：这里只列举出了MAC端支持的接口，详细说明请查看 FURenderer.h
